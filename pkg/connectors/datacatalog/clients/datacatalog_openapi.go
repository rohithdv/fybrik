...
func (m *openAPIDataCatalog) WriteAssetInfo(in *datacatalog.WriteAssetRequest, creds string) (*datacatalog.WriteAssetResponse, error) {
	resp, _, err := m.client.DefaultApi.WriteAssetInfo(context.Background()).XRequestDatacatalogWriteCred(creds).WriteAssetRequest(*in).Execute()
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("get asset info from %s failed", m.name))
	}
	return resp, nil
